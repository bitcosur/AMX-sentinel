...
class GovernanceObject(BaseModel):
    ...
    # sync amerod gobject list with our local relational DB backend
    @classmethod
    def sync(self, amerod):
        golist = amerod.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_amerod(amerod, item)
    ...
    @classmethod
    def import_gobject_from_amerod(self, amerod, rec):
        import decimal
        import amerolib
        import inflection
        ...
        # shim/amerod conversion
        object_hex = amerolib.SHIM_deserialise_from_amerod(object_hex)
        objects = amerolib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from amerod...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from amerod, with every run
        ...
        # get/create, then sync payment amounts, etc. from amerod - Amerod is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(amerod)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from amerod! %s" % e)
            govobj.vote_delete(amerod)
            return (govobj, None)
    ...
    def vote_delete(self, amerod):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(amerod, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, amerod, signal, outcome):
        import amerolib
        ...
        output = amerod.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = amerolib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(amerod, signal)

    def sync_network_vote(self, amerod, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = amerod.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = AMEROD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 amero addr, non-multisig
            if not amerolib.is_valid_amero_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Amero address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (amerolib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/AmeroDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import amerolib
        obj_data = amerolib.SHIM_serialise_for_amerod(self.serialise())
    ...
    def prepare(self, amerod):
        try:
            object_hash = amerod.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = AMEROD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not amerolib.is_valid_amero_address(addr, config.network):
    ...
    def hash(self):
        import amerolib
        return amerolib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = AMEROD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, amerod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - amerod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, amerod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - amerod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, amerod):
        now = int(time.time())
        return (self.created_at < (now - amerod.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, amerod):
        if self.is_expired(amerod):
            return False

        return True

    def is_deletable(self, amerod):
        if self.is_expired(amerod):
            return True
