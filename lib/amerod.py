...
class AmeroDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some amerod methods
    ...
    @classmethod
    def from_amero_conf(self, amero_dot_conf):
        from amero_config import AmeroConfig
        config_text = AmeroConfig.slurp_config_file(amero_dot_conf)
        creds = AmeroConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = amerolib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = amerolib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = amerolib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_amerod_proto_version_with_sentinel_ping)
