...
def is_valid_amero_address(address, network='mainnet'):
    ...
    amero_version = 140 if network == 'testnet' else 23
    ...
    try:
        ...
        # rescue from exception, not a valid Amero address
        return False

    if (address_version != amero_version):
        return False
...
# shims 'til we can fix the amerod side
def SHIM_serialise_for_amerod(sentinel_hex):
    from models import ameroD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = ameroD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in amerod
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # amerod expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    amerod_hex = serialise(obj)
    return amerod_hex


# shims 'til we can fix the amerod side
def SHIM_deserialise_from_amerod(amerod_hex):
    from models import ameroD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(amerod_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('amero.conf')
