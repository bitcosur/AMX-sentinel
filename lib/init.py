...
def has_amero_conf():
    ...
    valid_amero_conf = False

    # ensure amero_conf exists & readable
    #
    # if not, print a message stating that Amero Core must be installed and
    # configured, including JSONRPC access in amero.conf
    try:
        f = io.open(config.amero_conf)
        valid_amero_conf = True
    except IOError as e:
        print(e)

    return valid_amero_conf
...
def main():
    ...
    if not has_amero_conf():
        print("AmeroCore must be installed and configured, including JSONRPC access in amero.conf")
        sys.exit(1)
