...
@pytest.fixture
def valid_amero_address(network='mainnet'):
    return 'yYe8KwyaUu5YswSYmB3q3ryx8XTUu9y7Ui' if (network == 'testnet') else 'AXwKxUFPN1t7XCGkpDV17mVah5oFg257ao'


@pytest.fixture
def invalid_amero_address(network='mainnet'):
    return 'yYe8KwyaUu5YswSYmB3q3ryx8XTUu9y7Uj' if (network == 'testnet') else 'AXwKxUFPN1t7XCGkpDV17mVah5oFg257ax'
...
def test_valid_amero_address():
    from amerolib import is_valid_amero_address

    main = valid_amero_address()
    test = valid_amero_address('testnet')

    assert is_valid_amero_address(main) is True
    assert is_valid_amero_address(main, 'mainnet') is True
    assert is_valid_amero_address(main, 'testnet') is False

    assert is_valid_amero_address(test) is False
    assert is_valid_amero_address(test, 'mainnet') is False
    assert is_valid_amero_address(test, 'testnet') is True


def test_invalid_amero_address():
    from amerolib import is_valid_amero_address

    main = invalid_amero_address()
    test = invalid_amero_address('testnet')

    assert is_valid_amero_address(main) is False
    assert is_valid_amero_address(main, 'mainnet') is False
    assert is_valid_amero_address(main, 'testnet') is False

    assert is_valid_amero_address(test) is False
    assert is_valid_amero_address(test, 'mainnet') is False
    assert is_valid_amero_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from amerolib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % amerolib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(amerolib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(amerolib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(amerolib.blocks_to_seconds(16616)) == 2612035
